...
abstract class AbstractDataProviderConnector
{
    ...
    private const SOURCE_ATTRIBUTE_ATTRIBUTE_CONFIG_KEY = 'source';
    ...
    public function setConfig(array $config): void
    {
        foreach ($config['attributes'] ?? [] as $attributeConfig) {
            $this->attributeConfigEntries[$attributeConfig['name']] = $this->createConfigEntry($attributeConfig);
        }

        foreach ($config['local_data_attributes'] ?? [] as $attributeConfig) {
            $this->localDataAttributeConfigEntries[$attributeConfig['name']] = $this->createConfigEntry($attributeConfig);
        }
    }
    ...
    public function getItemById(string $id, array $filters = [], array $options = []): ?object
    {
        $preEvent = $this->createGetItemPreEvent();
        if ($preEvent !== null) {
            $preEvent->setId($id);
            $preEvent->setFilters($filters);
            $preEvent->setOptions($options);
            $this->eventDispatcher->dispatch($preEvent);
            $id = $preEvent->getId();
            $filters = $preEvent->getFilters();
            $options = $preEvent->getOptions();
        }

        $item = null;
        $itemData = $this->getItemDataById($id, $filters, $options);
        if ($itemData !== null) {
    ...
    public function getPage(int $currentPageNumber, int $maxNumItemsPerPage, array $filters = [], array $options = []): array
    {
        ...
        $pageItems = [];
        foreach ($this->getItemDataPage($currentPageNumber, $maxNumItemsPerPage, $filters, $options) as $itemData) {
            $pageItems[] = $this->createItemInternal($itemData);
        }
    ...
    protected function createItem(array $itemData): object
    {
        ...
        foreach ($this->attributeConfigEntries as $attributeConfigEntry) {
            $sourceAttributePath = $attributeConfigEntry[self::SOURCE_ATTRIBUTE_ATTRIBUTE_CONFIG_KEY];
            $dataValue = $itemData[$sourceAttributePath] ?? null;
            $normalizedItem[$attributeConfigEntry[self::NAME_ATTRIBUTE_CONFIG_KEY]] = $dataValue;
        }
    ...
    private function getRequestedLocalData(array $itemData, array $requestedLocalDataAttributes): array
    {
        ...
        foreach ($requestedLocalDataAttributes as $requestedLocalDataAttribute) {
            if (($localDataAttributeConfigEntry = $this->localDataAttributeConfigEntries[$requestedLocalDataAttribute] ?? null) !== null) {
                $attributeValue = $itemData[$localDataAttributeConfigEntry[self::SOURCE_ATTRIBUTE_ATTRIBUTE_CONFIG_KEY]] ?? null;
                if ($attributeValue !== null) {
                    $is_array_attribute = $localDataAttributeConfigEntry[self::IS_ARRAY_ATTRIBUTE_CONFIG_KEY];
                ...
                $localDataAttributes[$requestedLocalDataAttribute] = $attributeValue;
            }
        }
    ...
    abstract protected function getItemDataById(string $id, array $filters, array $options): ?array;

    abstract protected function getItemDataPage(int $currentPageNumber, int $maxNumItemsPerPage, array $filters = [], array $options = []): array;
